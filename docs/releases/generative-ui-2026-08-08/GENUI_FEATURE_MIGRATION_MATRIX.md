...
| Plugins domain/catalogue | `PluginCatalog`, persisted plugin records and UI | Capability Registry with migration aliases only | Migrate persisted data | Active New Haven routes use the v11 Capability Registry/editor and runtime `ActiveCapability`; legacy classes/adapters and data migration still prevent deletion. |
| Macros UI/data | Tasks/Actions UI and automation records | Tasks/capabilities/workflows without stale Macro system | Migrate then retire | Preserve useful workflows; explicit schema/data migration required. |
| Automations worker | `Haven.Automations`, `Haven.AutomationWorker` | Cross-platform Tasks/background execution | Preserve and extend | Closed-UI execution, permissions, Windows/Android commands required. |

...
Historical target: `results-day-phase2-worker8-haven-ui` (`97d44d54-c00a-4e9b-916d-42981df23fc4`). Current comparison: Worker 14 (`c44c4471-cb35-4af7-a5ca-3f58ccd562dc`).
...
| IM-5 | Native animated-image keyframes, rigging/bones/IK/handles | STILL RELEVANT — REQUIREMENT | Current Imagine has generic media timeline; no Imagine-native rig/keyframe model found. Global Haven.UI/Present animation is not a substitute. No unique historical rig code proven yet. |
...
| IM-8 | TTS Studio + custom censorship workflow | STILL RELEVANT — REQUIREMENT | No true current Imagine TTS Studio/censorship workflow found; no recoverable historical implementation proven yet. |
...
| W6-PROXY | Eliminate hidden `ElementProxy` runtime forwarding | NEEDS VERIFICATION | Large proxy catalog survives, but TopRail proves catalog presence alone does not mean runtime forwarding. Audit consumers before deleting/migrating. |
| W6-CONTEXT | Correct Haven-owned popup/context-menu surface | NEEDS VERIFICATION | Legacy `HavenContextMenu` consumers remain in shell/sidebar/browser/chat/automations. Classify consumer-by-consumer; no global replacement. |

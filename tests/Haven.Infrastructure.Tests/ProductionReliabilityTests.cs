...
public sealed class ProductionReliabilityTests : IDisposable
{
    ...
    public async Task PreMigrationBackupUsesSqliteBackupAndProducesVerifiedManifest()
    {
        ...
        var backup = await maintenance.PrepareForMigrationAsync(Migrations.All.Max(item => item.Version) + 1, CancellationToken.None);

        Assert.NotNull(backup);
        Assert.Equal(Migrations.All.Max(item => item.Version), backup!.FromVersion);
        Assert.Equal(Migrations.All.Max(item => item.Version) + 1, backup.ToVersion);
        ...
        Assert.Null(await maintenance.PrepareForMigrationAsync(Migrations.All.Max(item => item.Version) + 1, CancellationToken.None));
    }

...
public sealed class ProductionReliabilityTests : IDisposable
{
    ...
    public async Task PreMigrationBackupUsesSqliteBackupAndProducesVerifiedManifest()
    {
        ...
        var backup = await maintenance.PrepareForMigrationAsync(15, CancellationToken.None);

        Assert.NotNull(backup);
        Assert.Equal(14, backup!.FromVersion);
        Assert.Equal(15, backup.ToVersion);
        ...
        Assert.Null(await maintenance.PrepareForMigrationAsync(15, CancellationToken.None));
    }

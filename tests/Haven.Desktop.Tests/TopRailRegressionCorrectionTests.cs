...
public sealed class TopRailRegressionCorrectionTests
{
    ...
    public async Task Final_rail_restores_compact_tabs_chevrons_navigation_effort_palette_and_text_tabs()
    {
        ...
        try
        {
            window.Show(); rail.SetTabs([new TopRailTab("go", "Go", "sparkles", true, false)]); rail.SetNavigationAvailability(true, true); rail.SetModelSummary("mistral-small", 60); await Dispatcher.UIThread.InvokeAsync(() => { }); window.UpdateLayout();

...
public sealed class ImaginePage : UserControl, IDisposable
{
    ...
    {
        _projects = projects; _semantic = semantic; _assistant = assistant; _scene = new ImagineWorkspaceScene(); _host = new HavenSceneControl { Root = _scene.Root }; Content = _host;
    ...
    public async Task ImportPathAsync(string path)
    {
        ...
        await RunOperationAsync("Importing media…", async token => { var asset = await _projects.ImportAssetAsync(_session.Project.Id, path, kind.Value, token); _session.AddImportedAsset(asset); _scene.SetMode(asset.Kind); await _projects.SaveAsync(_session.Project, token); SetStatus($"Imported {asset.Name}. The source file was left unchanged."); });
    }

...
internal sealed partial class WordWriteHavenScene : IDisposable
{
    ...
    {
        if (block.Table is null || block.Table.Rows.Count == 0) return; var controls = Bar($"Write.Word.Table.{block.Id:N}.Controls", 0); foreach (var pair in new[] { ("+ Row", 1), ("− Row", 2), ("+ Column", 3), ("− Column", 4) }) { var button = Btn($"Write.Word.Table.{block.Id:N}.{pair.Item2}", pair.Item1); button.Invoked += (_, _) => { if (_editor is null) return; _editor.SelectBlock(block.Id); switch (pair.Item2) { case 1: _editor.AddTableRow(); break; case 2: _editor.RemoveTableRow(); break; case 3: _editor.AddTableColumn(); break; case 4: _editor.RemoveTableColumn(); break; } RebuildAll(); }; controls.Add(button); } card.Add(controls); var columns = block.Table.Rows.Max(row => row.Cells.Count); var grid = new Container { Name = $"Write.Word.Table.{block.Id:N}", Layout = HavenLayout.Grid, Columns = string.Join(' ', Enumerable.Repeat("1fr", columns)), Rows = string.Join(' ', Enumerable.Repeat("Auto", block.Table.Rows.Count)) }; grid.SetValue(HavenProperties.Gap, HavenLength.Px(3)); for (var row = 0; row < block.Table.Rows.Count; row++) for (var column = 0; column < block.Table.Rows[row].Cells.Count; column++) { var cell = block.Table.Rows[row].Cells[column]; var input = Field($"Write.Word.Table.Cell.{cell.Id:N}", $"Table cell {row + 1}, {column + 1}", "Cell"); input.Text = cell.Text; input.Multiline = true; input.SetValue(HavenProperties.Row, row); input.SetValue(HavenProperties.Column, column); input.SetValue(HavenProperties.MinHeight, HavenLength.Px(48)); if (row == 0 && block.Table.HeaderRow) input.SetValue(HavenProperties.FontWeight, 700); input.Invalidated += (_, _) => { if (_suppress) return; _editor?.SelectBlock(block.Id); _editor?.UpdateTableCell(cell.Id, input.Text); }; grid.Add(input); } card.Add(grid);
    }

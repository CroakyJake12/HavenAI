...
public sealed partial class CanvasPage : UserControl, IDisposable
{
    ...
    public CanvasPage(HavenEventBus bus, INotesRepository repository, INotesImportExportService formats)
    {
        ...
        _route.SurfaceSelectionChanged += OnSurfaceSelectionChanged; _route.DocumentOpenRequested += OnDocumentOpenRequested; _route.LibraryRequested += OnLibraryRequested; _route.BoardRequested += OnBoardRequested; _route.AddBoardRequested += OnAddBoardRequested; _route.BoardRenameRequested += OnBoardRenameRequested; _route.SpecialInsertRequested += OnSpecialInsertRequested;
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        try { await RefreshDocumentsAsync(cancellationToken); ShowLibrary(); _autosaveTimer.Start(); _bus.Fire("Canvas.Opened"); }
    ...
    private void SetDocument(NotesDocument document, int index) { Document = document; _documentIndex = Math.Clamp(index, 0, Math.Max(0, _documents.Count - 1)); _boardIndex = 0; _controller = new CanvasInteractionController(CanvasDocumentModel.GetBoard(document, _boardIndex)); _dirty = false; RefreshScene(); }
    ...
    private void RefreshScene() { if (Document is null || _controller is null) return; SyncBoardReference(); _route.ShowWorkspace(); _route.SetControllerDocument(Document, _controller, _documentIndex, _documents.Count, _controller.History.CanUndo, _controller.History.CanRedo); _route.SetBoards(CanvasDocumentModel.GetBoardTitles(Document), _boardIndex); }

/*
 * FILE DOCUMENTATION
 * Where: src/Haven.Browser/BrowserDownloadFilePolicy.cs, in the Browser layer, which isolates browser state, safety policy, transport, and automation.
 * What: This file owns BrowserDownloadFilePolicy. Read the type and member comments below as a map of each responsibility.
...
/// <summary>
/// Represents browser download file policy and keeps its related state and behavior together.
/// </summary>
public static class BrowserDownloadFilePolicy
{
    /// <summary>
    /// Stores maximum file name length locally so this component can preserve the dependency, cache, or state between member calls.
    /// </summary>
    public const int MaximumFileNameLength = 180;
    /// <summary>
    /// Stores partial file retention locally so this component can preserve the dependency, cache, or state between member calls.
    /// </summary>
    public static readonly TimeSpan PartialFileRetention = TimeSpan.FromHours(24);
    /// <summary>
    /// Stores partial marker locally so this component can preserve the dependency, cache, or state between member calls.
    /// </summary>
    private const string PartialMarker = ".haven-download-";

    /// <summary>
    /// Stores reserved windows names locally so this component can preserve the dependency, cache, or state between member calls.
    /// </summary>
    ...
    /// <summary>
    /// Performs the allocate unique path step owned by this component.
    /// </summary>
    ...
    /// <summary>
    /// Creates partial path with the invariants required by its callers.
    /// </summary>
    public static string CreatePartialPath(string destination) =>
        destination + PartialMarker + Guid.NewGuid().ToString("N") + ".tmp";

    /// <summary>
    /// Performs the cleanup stale partial files step owned by this component.
    /// </summary>
    ...
    /// <summary>
    /// Performs the truncate preserving extension step owned by this component.
    /// </summary>
    ...
    /// <summary>
    /// Performs the truncate runes step owned by this component.
    /// </summary>
    ...
    /// <summary>
    /// Performs the ensure confined step owned by this component.
    /// </summary>

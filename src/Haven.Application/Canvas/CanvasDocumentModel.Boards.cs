...
public static partial class CanvasDocumentModel
{
    ...
    public static void ReplaceBoard(NotesDocument document, NotesCanvasData replacement, int boardIndex)
    {
        ...
        var block = page.Blocks.First(candidate => candidate.Kind == NotesBlockKind.Canvas && ReferenceEquals(candidate.Canvas, current));
    ...
    {
        if (!IsCanvasDocument(document)) throw new InvalidOperationException("This Notes document is not a Haven Canvas board.");
    ...
    private static NotesCanvasData EnsureBoardOnPage(NotesPage page)
    {
        ...
        if (page.CanvasObjects.Count > 0)
        {
            ...
            foreach (var value in page.CanvasObjects) if (ids.Add(value.Id)) board.Objects.Add(value);
            page.CanvasObjects.Clear();
        }
        return board;
    }
